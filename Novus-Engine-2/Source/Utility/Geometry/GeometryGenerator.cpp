...
void GeometryGenerator::CreateSphere(float radius, uint32_t sliceCount, uint32_t stackCount, Mesh& mesh)
{
	mesh.Vertices.clear();
	mesh.Indices.clear();
...
void GeometryGenerator::CreateSphere(float radius, uint32_t sliceCount, uint32_t stackCount, SimpleMesh& mesh)
{
	mesh.Vertices.clear();
	mesh.Indices.clear();

	SimpleVertex topVertex(Vector3(0.0f, +radius, 0.0f), Vector3(0.0f, +1.0f, 0.0f));
	SimpleVertex bottomVertex(Vector3(0.0f, -radius, 0.0f), Vector3(0.0f, -1.0f, 0.0f));
...
void GeometryGenerator::CreateGeosphere(float radius, uint32_t numSubdivisions, Mesh& mesh)
{
...
void GeometryGenerator::CreateCylinder(float bottomRadius, float topRadius, float height, uint32_t sliceCount, uint32_t stackCount, Mesh& mesh)
{
	mesh.Vertices.clear();
	mesh.Indices.clear();
...
void GeometryGenerator::BuildCylinderTopCap(float bottomRadius, float topRadius, float height,
	uint32_t sliceCount, uint32_t stackCount, Mesh& mesh)
{
	...
	uint32_t centerIndex = (uint32_t)mesh.Vertices.size() - 1;
...
void GeometryGenerator::CreatePlane(float width, float depth, uint32_t m, uint32_t n, Mesh& mesh)
{
...
void GeometryGenerator::CreateSkybox(std::vector<Vector3>& vertices, std::vector<uint32_t>& indices)
{
	Vector3 v[24];
